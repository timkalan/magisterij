...
\label{izr:forking}
    ...
    podatke pri podpisni shemi, pri kateri se pri podpisovanju uporablja zgostitve delnih podpisov
    in sporočila prek slučajnih orakljev (npr.\ pri Shnorrovem podpisu je ta zgostitev izziv 
    $e = H(X || m)$). Če lahko $F$  z nezanemarljivo verjetnostjo vrne par $(m, \sigma)$, kjer je $\sigma$ 
    veljaven podpis sporočila $m$, potem lahko z nezanemarljivo verjetnostjo z enakimi vhodnimi
    podatki (enakim trakom v Turingovem stroju) in uporabo drugega oraklja, $F$ vrne dva para 
    $(m, \sigma)$ in $(m. \sigma')$, kjer podpisa temeljita na drugačnih zgostitvah.
    \todo{mogoče moram uporabiti definicijo, kjer se vračajo trojke}
\end{izrek}
...
\begin{dokaz}[Dokaz leme o razcepu]
    Naj bo $F$ napadalec, ki nima na voljo nobenega sporočila. Naj bo $k$ varnostni parameter obravnavane
    sheme za podpise. Denimo, da med svojim napadom napadalec $F$ slučajnemu oraklju $H$ pošlje največ
    polinomsko mnogo poizvedb v parametru $k$. Označimo poizvedbe, ki jih $F$ stori z $\mathcal{Q}_1,
    \mathcal{Q}_2, \dots, \mathcal{Q}_Q$ (brez škode za splošnost lahko prepdostavimo, da so poizvedbe
    med seboj različne). Odgovore oraklja označimo z $\rho_1, \rho_2 , \dots, \rho_Q$. 
    ...
    ($1 < \beta < Q$). Ker je poizvedb polinomsko mnogo v parametru $k$ in je verjetnost uspeha 
    nezanemarljiva, mora obstajati polinom $P(k)$, za katerega je verjetnost uspeha
    $$
    \text{Pr}_{\omega, \rho_1, \rho_2 , \dots, \rho_Q}(\text{$F$ vrne veljaven ponaredek}) \geq \frac{1}{P(k)},
    $$
    kjer je poizvedba $\mathcal{Q}_\beta$ res temeljila na sporočilu in delnem podpisu.
    ...
    $\rho_1, \dots, \rho_Q$ navzdol omejena z $1/2P(k)$. To nam da par $\beta$ in $\omega$, ki ga
    lahko uporabimo, da prek leme~\ref{lema:verjetnost} definiramo množico $R_{\beta, \omega}$, ki
    vsebuje tiste $(\rho_1, \dots, \rho_{\beta - 1})$, za katere je verjetnost uspeha po $(\rho_\beta,
    \dots. \rho_Q)$ večja od $1/4P(k)$.
