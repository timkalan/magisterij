...
\subsubsection{Dokazovanje varnosti: simulatorji}
Pogosto je cilj varnostnih dokazov pokazati, da je varnost neke sheme enakovredna težavnosti nekega
problema, za katerega domnevamo, da je težek. V takih primerih je pogosto uporabljena metoda
\textit{dokazovanja z redukcijo} (angl.\ \textit{reduction proof}). Ta metoda temelji na
predpostavki, da če bi napadalec lahko učinkovito napadel shemo, bi lahko tudi rešil težaven
problem.

V takih dokazih se pogosto uporabljajo \textit{simulatorji}. Simulator je algoritem, ki posnema
napadalca, vendar ne potrebuje nobenih skrivnih informacij, ki jih ima napadalec. Če lahko
simulator, omejen s polinomskim časom
...
Kot bomo videli, je za dokaz varnosti potrebno, da lahko algoritem, ki bo simuliral proces podpisovanja
in napadanja (pogosto se tak algoritem imenuje \textit{simulator}), v polinomskem času za vsakega
pokvarjenega podpisnika $P_j$ pridobi diskretni logaritem $I_j$ iz predloženega dokaza o znanju brez
razkritja znanja. Izkaže pa se, da če je $P_j$ dokaz izračuna s $q$ klici slučajnega oraklja, potem
simulator uspešno pridobi diskretni logaritem z verjetnostjo največ $1/q$. Še več, če je $k$ pokvarjenih
