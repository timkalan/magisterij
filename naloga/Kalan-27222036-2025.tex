...
\label{sec:osnove}
Preden si lahko pogledamo natančneje kako lahko skupina generira en sam podpis sporočila, si moramo 
...
\label{def:digisig}
    ...
    podpis pa leži v v končni množici podpisov $\Sigma$.
    \begin{itemize}
        \item $\mathcal{P}$ je algoritem za ustvarjanje javnih parametrov podpisa. Definira grupo $G$,
            ...
            uporabijo dobro poznane varne grupe.
...
Poleg parametrov $p, q$ in $g$, morata podpisnik in preverjevalec določiti oz.\ imeti dostop do iste varne 
...
generiran za vsako sporočilo znova (glej opombo~\ref{opomba:nonce}).
...
\label{opomba:nonce}
    ...
    $$
    y_1 - y_2 \equiv e_1 s + r_1 - e_2 s - r_2 \equiv (e_1 - e_2)s + (r_1 - r_2) \pmod q.
    $$
...
\subsection{Schnorrov podpis v splošnih grupah}
Čeprav je Schnorr originalno~\cite{schnorr1989sig} opisal podpis v multiplikativnih grupah naravnih
števil modulo $p$ $\Z_p^*$, pri njem zares ni nič, kar bi specifično delalo samo v teh grupah. Zares
je Schnorrov podpis mogoče posplošiti na katerekoli končne grupe, kjer obstaja učinkovit algoritem
za množenje in je problem diskretnega logaritma težek.

Naj bo grupa $G$ taka končna grupa reda $p$. V njej si izberemo element $g$ reda $q$, kjer mora
veljati, da je $q$ dovolj velik (velikost je odvisna od varnostnega parametra). Pri osnovni verziji
je tu moralo veljati, da $q$ deli $p - 1$, v splošnem pa sta lahko $p$ in $q$ tudi enaka. Efektivno
grupo, v kateri računamo določa $q$, tako da se v tem primeru navadno predpostavi, da je red grupe
$G$ kar $q$. Za generiranje ključa si potem izberemo naključno število $s \in [0, q - 1]$ in izračunamo
$$
I = g^s
$$
...
Izračun podpisa poteka podobno, kot pri osnovni verziji. Najprej si izbreremo naključno število
$r \in [0, q - 1]$ in izračunamo
$$
X = g^r.
$$
Potem z uporabo zgoščevalne funkcije $H : \{0, 1\}^* \rightarrow \Z_q$ izračunamo $e = H(X || m)$,
drugi del podpisa pa izračunamo kot
$$
y = es + r.
$$

Preverjanje podpisa poteka praktično enako kot prej, ob predloženem podpisu $(X', y')$ sporočila $m$
je potrebno izračunati $e = H(X' || m)$ in preveriti, če velja
$$
g^{y'} \stackrel{?}{=} X' \cdot I^{e'}.
$$

Schnorrov podpis je tako precej splošno orodje, v zadnjem času je precej popularna uporaba Schnorrovih
podpisov v \textit{eliptičnih grupah}. Te omogočajo izbiro manjših parametrov, kar naredi podpis bolj
...
se da dokazati v standardnem modelu?}
