...
dokaz varnosti za podpisno shemo v tem modelu. Zamislili so si formalni model za večstranske
...
večstranske podpise, predstavljene v razdelku~\ref{sec:multisig}. Ideja oz.\ cilj večstranskega
...
preverljiv dokaz strukture $S$ in dejstva, da vsak član skupine soglaša s sporočilom $m$. V tem
razdelku predstavimo osnovno idejo ASM.
...
\label{def:asm}
    ...
    identifikacijsko številko (eno od števil $1, \dots, L$) in \textit{varnostni parameter} $k$,
    ...
    \begin{itemize}
        ...
        \item $\mathcal{S}$ je protokol za podpisovanje. Pognan je vsakič, ko neka podskupina 
            ...
            izvedbi lahko eden od članov objavi podpis $\sigma$.
...
v času podpisovanja ne morejo z gotovostjo vedeti, s kom podpisujejo (ne morejo zaupati, da res
sodelujejo s člani podskupine $S$, saj napadalec nadzira komunikacijo).
...
delijo samo varnostni parameter $k$ in slučajnega oraklja $H$, morajo za generiranje parametrov uporabiti
...
\label{primer:gen-zp}
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}
            ...
            \State $p \gets 2q + 1$
            \While{$q$ ni praštevilo ali $p$ ni praštevilo}
                \State $q \gets \text{naključno izbrano $k$-bitno število}$
                \State $p \gets 2q + 1$
            \EndWhile
            \State \Return{$p, q$}
...
enačbi za preverjanje~\eqref{eq:naive-ver} tako izničili vsi ostali javni ključi:
\begin{align*}
...
obravnavamo model slučajnega oraklja in uporabimo Fiat-Shamirjevo hevristiko~\ref{sec:fiat-shamir}.
...
Da ponovno pridobimo učinkovit podpis, lahko uporabimo slučajnega oraklja in kriptografsko
...
Sedaj imamo vse potrebno, da dokažemo ekvivalentnost šibke varnosti in varnosti, in s tem
...
poenostavljenega napadalca $\mathcal{F}$ nazaj na začetek podpisovanja z enakim virom naključnih bitov
...
poizvedbi. Potek obeh pogonov je prikazan na sliki~\ref{fig:forking_lemma}.

\begin{figure}[ht]
    \centering
\begin{tikzpicture}[
    node distance=0.5cm and 0.5cm,
    event_box/.style={rectangle, draw, minimum height=8mm, align=center},
    arrow_style/.style={->, >=Stealth, auto, semithick},
    fork_point/.style={rectangle, rounded corners, draw, thick, align=center, inner sep=3pt, text width=2.5cm},
]

% --- Run 1 ---
\node[event_box] (q1_run1) {Podpisovalne \\ poizvedbe \\ $1, \dots, r-1$};
\node[event_box, right=of q1_run1] (qr1_run1) {Podpisovalna \\ poizvedba $r$ \\ (1. krog)};
\node[fork_point, fill=red!15, draw=red, right=of qr1_run1] (j0_run1) {Zgoščevalna poizvedba $j_0$ \\ Odgovor: $e_{j_0}$};
\node[event_box, right=of j0_run1] (qr2_run1) {Podpisovalna \\ poizvedba $r$ \\ (2. krog)};
\node[event_box, right=of qr2_run1] (f1) {Prvi \\ Ponaredek};

\draw[arrow_style] (q1_run1) -- (qr1_run1);
\draw[arrow_style] (qr1_run1) -- (j0_run1);
\draw[arrow_style] (j0_run1) -- (qr2_run1);
\draw[arrow_style] (qr2_run1) -- (f1);

% --- Run 2 ---
\node[event_box, below=1.8cm of q1_run1] (q1_run2) {Podpisovalne \\ poizvedbe \\ $1, \dots, r-1$};
\node[event_box, right=of q1_run2] (qr1_run2) {Podpisovalna \\ poizvedba $r$ \\ (1. krog)};
\node[fork_point, fill=blue!15, draw=blue, right=of qr1_run2] (j0_run2) {Zgoščevalna poizvedba $j_0$ \\ Odgovor: $e'_{j_0}$};
\node[event_box, right=of j0_run2] (qr2_run2) {Podpisovalna \\ poizvedba $r$ \\ (2. krog)};
\node[event_box, right=of qr2_run2] (f2) {Drugi \\ ponaredek};

\draw[arrow_style] (q1_run2) -- (qr1_run2);
\draw[arrow_style] (qr1_run2) -- (j0_run2);
\draw[arrow_style] (j0_run2) -- (qr2_run2);
\draw[arrow_style] (qr2_run2) -- (f2);

% --- Fork Connection ---
\draw[dashed, gray, thick] (j0_run1.south) -- (j0_run2.north) 
    node[midway, right, font=\small, color=gray] {Razcep};
...
\begin{opomba}
    ...
    primeru lahko napadalec sodeloval pri več podpisovalnih sejah hkrati. Previjanje v tej situaciji
...
$$
Če preuredimo dobljeno enačbo, dobimo
$$
...
nezanemarljivo verjetnostjo vrne odgovor na problem diskretnega logaritma. Ker predpostavljamo, da
