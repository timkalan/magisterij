...
pomeni, da lahko njihovo veljavnost preverimo z uporabo Schnorrove enačbe za preverjanje
...
MuSig2, ki so ga leta $2020$ predstavili Nick, Ruffing in Seurin~\cite{jonas2020musig2}. Pred pojavom
...
\begin{itemize}
    ...
    \item V primerjavi z opisanim večstranskim Schnorrovim podpisom, MuSig2 ne omogoča prilagodljivosti
        ...
        podpis priča o skupnem strinjanju, preverjevalec ne more izvedeti, kdo je dejansko podpisoval.
        MuSig2 torej omogoča večjo zasebnost podpisnikov, kar je za nekatere aplikacije zaželena lastnost.
\end{itemize}
...
konstrukcijo Merklovega drevesa. To je zahtevalo veliko komunikacije med podpisniki, celo linearno
...
in agregacije, ki ju označimo $\mathcal{S}_i$ in $\mathcal{S}\mathcal{A}_i$ (za $i = 1, 2$). Po
...
\begin{itemize}
    ...
    \item \textbf{Drugi krog}:
        Vsak podpisnik na tem mestu potrebuje svoj zasebni ključ $s_i$, agregirani javni ključ
        $\tilde{I}$, ki ga lahko izračuna z algoritmom $\mathcal{A}$ in svoj koeficient agregacije
        $c_i$. Prav tako potrebujejo rezultat prvega kroga, torej zaveze $\{X_j | 1 \le j \le \nu\}$
...
oraklja, ki nam lahko pove $t$ rešitev problema diskretnega logaritma, mi pa želimo rešiti $t + 1$
...
dodatno kompleksnejši, ker dokazuje varnost tudi v primeru, da podpisniki sodelujejo pri več podpisih naenkrat,
