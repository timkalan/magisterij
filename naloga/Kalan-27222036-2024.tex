...
\textit{weak adversary}) $F'$. Ta še vedno izvaja napad z izbranim sporočilom in podskupino, 
ima pa bistveno manj privilegijev, kot prvotni nasprotnik $F$.

Šibek nasprotnik deluje podobno kot $F$, le da ne more na katerikoli točki pokvariti igralcev. To
mora storiti pred začetkom podpisovanja. $F'$ si torej pred podpisovanjem izbere množico igralcev,
in jih pokvari. Nadzira torej komunikacije, ki jih ti igralci prejmejo, in bere komunikacije, ki
jih pošljejo. Med drugim lahko za vhode v algoritme teh igralcev izbere poljubne vrednosti. Omenimo
še, da je pokvarjenih igralcev lahko največ $l - 1$, sicer ne ostane več noben legitimen podpisnik,
in napad nima smisla.

Kot za napadalce $F$, tudi tu definiramo, kaj pomeni, da je shema varna.
...
\begin{izrek}
    Naj bo $Q$ tak polinom, da je katerkoli varnostni parameter $k$ za velikost $L$ skupine podpisnikov
    $G$ velja
    $$
    L < Q(k).
    $$
...
\begin{dokaz}
    ...
    ki jih ima napadalec $F'$ (in še mnogo več), potem podpis tudi ni varen.

    $(\Leftarrow)$: Predpostavimo, da podpis ni varen.
\end{dokaz}
