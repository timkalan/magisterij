...
\begin{frame}
    ...
    \begin{center}
        \large
        Preprosto, elegantno, \textbf{težavno?}
    \end{center}
    \note[item]{30 s}
...
\begin{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.5\textwidth}
            \begin{center}
                ...
                \Large\[ I_O = (I_A \cdot I_B)^{-1} \cdot g^{s_O} \]
            \end{center}
        \end{column}
    \end{columns}
    ...
    \begin{block}{Rezultat: Agregiran javni ključ}
        \large
        \[ \tilde{I} = \underbrace{(I_A \cdot I_B)}_{\text{Anita \& Bojan}} \cdot \underbrace{\left((I_A \cdot I_B)^{-1} \cdot g^{s_O}\right)}_{\text{Oskar}} = g^{s_O} \]
    \end{block}
    % \begin{center}
...
\begin{frame}
    ...
    \begin{itemize}
        \item Vsak podpisnik dokaže, da pozna svoj zasebni ključ.
        \item Prepreči napad na generiranje ključev.
        \item Standarden Schnorrov podpis, ki dokazuje lastništvo.
    \end{itemize}
    \vspace{1cm}
    \begin{figure}
...
\begin{frame}[fragile]
    \frametitle{Nadgradnja}
    % \framesubtitle{Učinkovita zaveza skupini}

    \begin{center}
        \large\color{red!80!black}
        Preverjanje $L$ dokazov znanja je neučinkovito.
    \end{center}
    ...
    \begin{figure}
        ...
        \small Dokazilo o pripadnosti ključa $I_3$ z \color{blue!60} overjevalno potjo.
    \end{figure}
    \begin{center}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[
            ...
            \draw[<-, very thick, blue!60!black] ([yshift=-2.5cm]signers.east) -- ([yshift=-2.5cm]coord.west)
                ...
                node[midway, below, black, align=center] {$\prod X_i$};
            ...
            \draw[->, very thick, blue!60!black] ([yshift=-4cm]signers.east) -- ([yshift=-4cm]coord.west)
                ...
                % node[midway, below, black] {$y_i = H(\prod X_i, m, S)s_i + r_i$};
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \vspace{0.5cm}
        \item \textbf{Redukcija} na težavnost \textbf{problema diskretnega logaritma}.
        \vspace{0.5cm}
...
\begin{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.5\textwidth}
            \begin{block}{Novi cilji}
                \begin{itemize}
                    \item \textbf{Hitrost:} 2 kroga komunikacije
                    \item \textbf{Zasebnost:} Zamenljivost s standardnim podpisom
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
    ...
    \large Menjava \textbf{odgovornosti} za \textbf{učinkovitost} in \textbf{zasebnost}.
    \note[item]{30 s}
...
\begin{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.55\textwidth}
            \begin{itemize}
                \item \textbf{Hitrost:} Zahteva samo 2 kroga komunikacije.
                \item \textbf{Učinkovitost:} Uporablja agregacijo ključev.
                \item \textbf{Združljivost:} Rezultat je standarden Schnorrov podpis.
...
\begin{frame}
    ...
    \begin{columns}[T]
        \begin{column}{0.45\textwidth}
            \begin{block}{Varna agregacija ključev}
                Napad na generiranje ključev je preprečen s koeficienti $c_i$.%, ki so odvisni od \textbf{vseh} ključev v skupini.
            \end{block}
            \centering
        ...
        \begin{column}{0.55\textwidth}
            \begin{block}{Podpisovanje v 2 krogih}
                \vspace{0.3cm}
                \centering
                \begin{tikzpicture}[
...
\begin{frame}
    ...
    \renewcommand{\arraystretch}{1.5} % Add vertical padding to table rows
    \begin{table}
        \centering
        % \caption{Primerjane sheme}
        \begin{tabular}{l c l}
            \toprule
            Osnova & $\rightarrow$ & Schnorr \\
            Odgovornost & $\rightarrow$ & Shema ASM \\
            Učinkovitost & $\rightarrow$ & MuSig2 \\
            \bottomrule
        \end{tabular}
    \end{table}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.6\textwidth}
            \centering
            \vspace{0.66cm}
            \large\textit{... v odvisnosti od števila podpisnikov.}
        \end{column}
    \end{columns}
    \note[item]{30 s}
    ...
    \note[item]{Primerjal sem tri ključne pristope: osnovni Schnorr, shemo ASM za odgovornost in MuSig2 za učinkovitost.}
    ...
    \note[item]{Vse meritve so bile izvedene v odvisnosti od naraščajočega števila podpisnikov. Poglejmo si rezultate.}
\end{frame}
\begin{frame}
    ...
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            ...
            \textbf{\color{green!50!black}\large\bfseries ASM} je najpočasnejši zaradi kompleksne priprave, ki se izvede le enkrat.% (dokazi znanja, Merklovo drevo), ki se izvede le enkrat. 
            \end{block}
        \end{column}
        ...
        \begin{column}{0.5\textwidth}
            ...
            \begin{block}
                \small
                \textbf{\color{blue!60!black}\large\bfseries MuSig2} zahteva več računskega dela, a je v praksi bistveno hitrejši.% zaradi manj komunikacije (2 vs. 3 krogi). 
            \end{block}
        \end{column}
    \end{columns}
    \note[item]{60 s}
...
\begin{frame}
    ...
    \begin{columns}[T,totalwidth=\textwidth]
        \begin{column}{0.33\textwidth}
            \centering
            \color{orange!80!black}\large\bfseries Schnorr
            \rmfamily\normalsize
            \begin{itemize}
                \item linearna rast
                \item ni skalabilnosti
            \end{itemize}
        \end{column}
        
        \begin{column}{0.33\textwidth}
            \centering
            \color{green!50!black}\large\bfseries ASM
            \rmfamily\normalsize
            \begin{itemize}
                \item počasna rast
                \item učinkovitost
        ...
        \begin{column}{0.33\textwidth}
            \centering
            \color{blue!60!black}\large\bfseries MuSig2
            \rmfamily\normalsize
            \begin{itemize}
                \item konstanten čas
                \item idealna rešitev
            \end{itemize}
        \end{column}
    \end{columns}
    \note[item]{60 s}
...
\begin{frame}
    ...
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            \centering
            \textbf{Izberi ASM, ko ...}
            \\[0.5cm] % Ends the line and adds 0.3cm of space
        ...
        \begin{column}{0.5\textwidth}
            ...
            \begin{beamercolorbox}[sep=0.3cm,ht=5cm]{block body}
                \begin{itemize}
                    ...
                    \item ... potrebuješ združljivost z obstoječo Schnorr infrastrukturo (npr. Bitcoin).
                \end{itemize}
            \end{beamercolorbox}
        \end{column}
    \end{columns}
...
\begin{frame}
    \frametitle{Nadaljnje raziskovanje}
    % \framesubtitle{Odgovornost ali učinkovitost?}
    \begin{itemize}
