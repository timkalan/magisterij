...
\begin{frame}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Osnova}: Schnorrov podpis
        \note[item]{Teoretična osnova: Predstavitev Schnorrovega podpisnega algoritma.}
        \item \textbf{Prvi pristop}: Večstranski Schnorrov podpis (ASM)
...
\begin{frame}
    ...
    \vspace{0.5cm}
    \begin{itemize}
        \item \textbf{Učinkovitost:} En sam, kratek podpis. Hitro preverjanje, neodvisno od števila podpisnikov.
        \item \textbf{Minimalna komunikacija:} Čim manj krogov izmenjave sporočil med podpisniki.
        \bigskip
        \item \textbf{Odgovornost:} Možnost natančne identifikacije dejanskih podpisnikov.
        \item \textbf{Prilagodljivost:} Podpisovanje s strani poljubne podskupine članov.
    \end{itemize}
    \note[item]{60 s}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.55\textwidth}
            \begin{itemize}
                ...
                \end{itemize}
                \vspace{0.5cm}
                \item \textbf{Analogija}: Deluje kot fizično podpisovanje pogodb.
                \vspace{0.5cm}
...
\begin{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.5\textwidth}
            \begin{center}
                ...
                \vspace{0.5cm}
                Počaka in zgradi svoj ključ:
                \Large\[ I_M = (I_A \cdot I_B)^{-1} \cdot g^{s_M} \]
            \end{center}
        \end{column}
    \end{columns}
    ...
    \note[item]{Na desni pa imamo zlonamerno Oskarja. On ne ustvari svojega ključa naključno. Počaka, da vidi njuna ključa, in nato svojega zgradi namensko – tako, da bo izničil ostale.}
    \note[item]{In kaj je rezultat? Ko izračunamo agregiran ključ celotne skupine, se, kot vidite v spodnji enačbi, del od Anite in Boba popolnoma pokrajša z inverznim delom v Oskarjevem ključu.}
...
\begin{frame}
    \frametitle{Dokazi znanja brez razkritja znanja}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \vspace{0.5cm}
        \item Varnost se z redukcijo prevede na \textbf{težavnost problema diskretnega logaritma}.
        \vspace{0.5cm}
    ...
    \begin{columns}[T]
        \begin{column}{0.55\textwidth}
            \begin{itemize}
                ...
                \item \textbf{Učinkovitost:} Vsi javni ključi se združijo v \textbf{enega samega} (agregacija ključev).
                \item \textbf{Združljivost:} Rezultat je \textbf{standarden Schnorrov podpis}, zamenljiv z navadnim.
                \item \textbf{Zasebnost:} Podpis je nerazločljiv od podpisa posameznika, kar skrije člane skupine.
            \end{itemize}
        \end{column}
        \begin{column}{0.45\textwidth}
            \centering
            \begin{tikzpicture}[
                ...
                \draw[->, very thick, double distance=2pt] (0, 0.7) -- (0, -0.7) node[midway, right] {MuSig2};
    ...
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            \begin{block}{1. Varna agregacija ključev}
                Napad na generiranje ključev je preprečen z \textbf{utežmi} $c_i$, ki so odvisne od \textbf{vseh} ključev v skupini.
            \end{block}
            \centering
        ...
        \begin{column}{0.5\textwidth}
            \begin{block}{2. Podpisovanje v 2 krogih}
                ...
                \begin{tikzpicture}[
                    ...
                    \draw[dashed] (signers.south) -- ++(0,-4);
                    \draw[dashed] (coord.south) -- ++(0,-4);
                    
                    \draw[->, thick] ([yshift=-1cm]signers.east) -- node[midway, above, font=\small] {1. Krog (Zaveza)} node[midway, below, font=\small] {$X_i$} ([yshift=-1cm]coord.west);
                    \node[align=center, font=\tiny\itshape] at ([yshift=-1.7cm]signers) {(lahko vnaprej)};

                    \draw[->, thick] ([yshift=-3cm]signers.east) -- node[midway, above, font=\small] {2. Krog (Odgovor)} node[midway, below, font=\small] {$y_i$} ([yshift=-3cm]coord.west);
                \end{tikzpicture}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}
    ...
    \begin{block}{Primerjani pristopi}
...
\begin{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.5\textwidth}
            \centering
            \textbf{2. Podpisovanje (samo računanje)}
...
\begin{frame}
    ...
    \begin{columns}[T,totalwidth=\textwidth]
        \begin{column}{0.33\textwidth}
            ...
            \rmfamily\normalsize
            \begin{itemize}
                \item Čas narašča \textbf{linearno} ($O(L)$).
                \item Ni skalabilno.
            \end{itemize}
        \end{column}
        ...
        \begin{column}{0.33\textwidth}
            ...
            \rmfamily\normalsize
            \begin{itemize}
                \item Rast je \textbf{zelo počasna} ($O(\log L)$).
                \item Zelo učinkovito.
            \end{itemize}
        \end{column}
        ...
        \begin{column}{0.33\textwidth}
            ...
            \rmfamily\normalsize
            \begin{itemize}
                \item Čas je \textbf{konstanten} ($O(1)$).
                \item Idealno.
            \end{itemize}
        \end{column}
    \end{columns}
